...
1. Read the root `AGENTS.md`, `.mat/AGENTS.md`, and every file it routes for feature planning, including `.mat/CONTEXT.md`, `.mat/WORKFLOW.md`, `.mat/TASKS.md`, `.mat/templates/feature.md`, `.mat/templates/task.md`, and all existing Feature and Task Specifications under `.mat/features/`.
...
1. Convert the agreed discovery into a clear, audience-neutral Feature Specification using every heading in `.mat/templates/feature.md`. Keep its Executive Summary reusable in email, a work-tracking system, or stakeholder communication. Exclude internal AI-process commentary and implementation decomposition from the Feature Specification.
...
3. Create each proposed Task from `.mat/templates/task.md`. Distinguish requirements and approved constraints from implementation preferences. Include observable Acceptance Criteria, proportionate testing expectations, relevant dependencies, and enough verified technical context for the Software Engineer to implement without repeating discovery.
...
3. Create `.mat/features/<FFFF-feature-name>/FEATURE.md` from `.mat/templates/feature.md`, mark it `Ready for Tasking`, and create its `tasks/` directory.
...
5. Add the Feature exactly once to `.mat/TASKS.md` using the heading `### [FFFF Feature Name](features/FFFF-feature-name/FEATURE.md)` and `Status: Ready for Tasking`. List every Task beneath it exactly once in implementation order with its relative link and inline `Ready` state. Preserve unrelated Features, Tasks, states, and ordering. Do not populate Active Task or mark any Task `In Progress`.

Do not implement code or invoke the Software Engineer or QA Engineer.

## Report

Report the Feature Specification path, ordered Task paths, index update, important dependencies, and confirmed scope boundary. Recommend `mat-build` for the first Task without starting implementation.
