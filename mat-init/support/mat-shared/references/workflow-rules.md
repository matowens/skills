...
- `mat-retro` begins only after Mat confirms the Feature was merged, deployed, and validated. Before changing workflow state, it requires a clean worktree, returns to the configured target branch, updates it from the remote with a fast-forward-only pull, and safely deletes the merged local Feature branch when Git permits. It then moves the Feature from `Deployment` to `Retrospective`, reconciles approved follow-up, marks it `Complete`, and collapses its expanded Task group in `.mat/TASKS.md` to one linked completed-Feature entry.
...
After Mat confirms merge, deployment, and production validation, `mat-retro` owns local branch closeout. It fetches the configured remote with pruning, switches to the target branch, and pulls that branch using `--ff-only`. A dirty worktree or failed fetch, switch, or pull leaves the Feature in `Deployment`. Delete the merged review request's local source branch only with `git branch -d`; never force-delete it. A refused safe deletion is reported and preserved but does not block retrospective once the target branch is current.
