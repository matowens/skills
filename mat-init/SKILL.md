...
3. Read [project structure](support/mat-shared/references/project-structure.md), [workflow rules](support/mat-shared/references/workflow-rules.md), and every file in [canonical templates](support/mat-shared/templates/), including the Project Idea Bin and retrospective template.
...
3. Exact command map: dependency installation, framework CLI commands such as Artisan, targeted and full tests, formatting, static analysis, frontend tooling, builds, migrations, and other routine project operations. Record the working directory and complete invocation rather than a shorthand command that would fail from the host. Capture recurring shell, container, quoting, or diagnostic behavior and its proven fallback. Record optional capabilities such as Laravel Boost only when installed; never make them a prerequisite for a dependable base command.
4. Team and delivery: work tracker, mapping between this workflow's Feature and Task terms and the team's agile terminology, branch and review-request expectations, human review or Definition of Done, CI, hosting environments, deployment, release, and rollback practices.
5. Source control and deployment preparation: inspect the configured remotes and confirm GitLab or GitHub, remote name and URL, target branch, `glab` or `gh`, CLI installation and authentication status, Mat's provider identity or verified self-assignment, ready-versus-draft convention, applicable templates, labels, and reviewers, and whether final `mat-next` may push the Feature branch and create or update its review request. Never record credentials or tokens.
6. Project-specific safety, data, access, integration, or operational risks that future feature work must know.

Do not ask Mat to repeat low-risk facts that the repository establishes clearly. Do ask for confirmation when local execution, container selection, credentials or access, delivery behavior, or team process cannot be proven from files. Before writing the final context, present a concise summary of `Verified`, `Confirmed by Mat`, and `Unknown` facts. Require Mat to confirm or correct the Local Development Environment, Command Map, Team Workflow and Terminology, Source Control and Deployment Preparation, and Hosting and Delivery sections; allow an item to remain `Unknown` only when Mat explicitly accepts that gap.
...
2. Update every section of the generated `.mat/CONTEXT.md` with verified repository facts and Mat's confirmed answers. Preserve the execution boundary and exact invocation for every important command. Record verified recurring environment behavior under Known Environment Behaviors, including optional accelerators and reliable fallbacks. Keep explicitly accepted unresolved facts marked `Unknown`.
